...
var HKVariantsRev = "媪 媼|缽 鉢|才 纔|吃 喫|痴 癡|唇 脣|囱 囪|葱 蔥|潀 潨|兑 兌|敚 敓|鈎 鉤|枴 柺|媯 嬀|核 覈|户 戶|稜 棱|秘 祕|啟 啓|鋭 銳|叁 叄|濕 溼|涚 涗|税 稅|説 說|台 臺|抬 擡|枱 檯|蜕 蛻|捝 挩|脱 脫|腽 膃|溈 潙|偽 僞|蒍 蔿|為 爲|衞 衛|温 溫|榅 榲|輼 轀|鰛 鰮|揾 搵|卧 臥|敍 敘|煙 菸|踴 踊|悦 悅|閲 閱|蒀 蒕|氲 氳|煴 熅|愠 慍|緼 縕|醖 醞|藴 蘊|灶 竈|皂 皁|針 鍼|衹 只|眾 衆|粧 妝|棁 梲|粽 糉|糭 糉";

var from_hk = [HKVariantsRevPhrases, HKVariantsRev];
...
var TWVariantsRev = "痺 痹|缽 鉢|才 纔|參 蔘|吃 喫|痴 癡|床 牀|唇 脣|潀 潨|顎 齶|峰 峯|媯 嬀|核 覈|韁 繮|稜 棱|裡 裏|么 幺|秘 祕|麵 麪|鯰 鮎|啟 啓|群 羣|抬 擡|溈 潙|偽 僞|蒍 蔿|為 爲|汙 污|嫻 嫺|洩 泄|簷 檐|睪 睾|踴 踊|灶 竈|皂 皁|針 鍼|眾 衆|著 着|粽 糉";

var from_tw = [TWVariantsRevPhrases, TWVariantsRev];

var TWPhrasesRev = "阿拉伯聯合大公國 阿拉伯聯合酋長國|鑀 鎄|安地卡及巴布達 安提瓜和巴布達|巴貝多 巴巴多斯|巴布亞紐幾內亞 巴布亞新幾內亞|半形 半角|鉳 錇|貝里斯 伯利茲|貝南 貝寧|比特幣 比特幣|筆記型電腦 筆記本電腦|壁紙 壁紙|變數 變量|標頭檔案 頭文件|表示式 表達式|賓士 奔馳|冰棒 冰棍|波士尼亞赫塞哥維納 波斯尼亞黑塞哥維那|波札那 博茨瓦納|波長分波多工 波分複用|撥出 呼出|帛琉 帕勞|鈽 鈈|布吉納法索 布基納法索|布林 布爾|部落格 博客|埠 端口|菜單 菜單|參數列 參數表|查德 乍得|查詢 查找|場效電晶體 場效應管|超程式設計 元編程|巢狀 嵌套|程式 程序|程式碼 代碼|程式設計 編程|程式設計師 程序員|程式語言 編程語言|程序 進程|程序不正義 程序不正義|程序導向 面向過程|程序式程式設計 過程式編程|程序正義 程序正義|喫茶小舖 喫茶小舖|除錯 調試|儲存 保存|處理程序 處理程序|觸控 觸摸|觸控式螢幕 觸摸屏|傳送 發送|純喫茶 純喫茶|磁碟 磁盤|磁碟機代號 盤符|磁軌 磁道|伺服器 服務器|叢集 集羣|存檔 存盤|存取 訪問|鎝 鍀|打開 打開|代碼 代碼|單核心 宏內核|宕機 死機|檔案 文件|檔名 文件名|刀鋒伺服器 刀片服務器|地址 地址|登出 註銷|登錄檔 註冊表|登入 登錄|低級 低級|低階 低級|遞迴 遞歸|點選 點擊|點陣圖 位圖|電晶體 晶體管|電腦保安 計算機安全|電腦科學 計算機科學|調變 調制|調色盤 調色板|碟片 盤片|定址 尋址|丟擲 拋出|都會網路 城域網|堆疊 堆棧|對話方塊 對話框|對象 對象|對映 映射|多工 多任務|多明尼加 多米尼加|多型 多態|多執行緒 多線程|砈 砹|厄瓜多 厄瓜多爾|厄利垂亞 厄立特里亞|二極體 二極管|鍅 鈁|防毒 殺毒|防寫 寫保護|訪問 訪問|非同步 異步|飛航模式 飛行模式|分割槽 分區|分空間多重進接 空分多址|分碼多重進接 碼分多址|分頻多工 頻分複用|分頻多重進接 頻分多址|分散式 分佈式|分時多工 時分複用|分時多重進接 時分多址|副檔名 擴展名|傅立葉 傅里葉|複製 拷貝|蓋亞那 圭亞那|甘比亞 岡比亞|感測 傳感|高畫質 高清|高級 高級|高階 高端|高空彈跳 蹦極|高效能運算 高性能計算|哥斯大黎加 哥斯達黎加|格瑞那達 格林納達|葛摩 科摩羅|工作管理員 任務管理器|工作列 任務欄|瓜地馬拉 危地馬拉|關聯式資料庫 關係數據庫|光碟 光盤|光碟機 光驅|過載 重載|哈薩克 哈薩克斯坦|海內存知己 海內存知己|駭客 黑客|函式 函數|函數語言程式設計 函數式編程|核心 內核|覈取按鈕 複選按鈕|覈取方塊 複選框|宏都拉斯 洪都拉斯|後設資料 元數據|呼叫 調用|互動 交互|互動式 交互式|滑鼠 鼠標|畫素 像素|黃宏 黃宏|回撥 回調|迴圈 循環|匯出 導出|匯流排 總線|匯入 導入|彙編 彙編|稽覈 審覈|機率 概率|積體電路 集成電路|吉布地 吉布堤|吉里巴斯 基里巴斯|幾內亞比索 幾內亞比紹|技術長 首席技術官|計程車 出租車|記憶體 內存|加彭 加蓬|迦納 加納|價效比 性價比|剪貼簿 剪貼板|剪下 剪切|檢視 查看|簡報 演示文稿|簡訊 短信|建構函式 構造函數|建立 創建|腳踏車 自行車|結束通話 掛斷|截圖 截屏|解除安裝 卸載|解構函式 析構函數|解析度 分辨率|介面 接口|介面卡 適配器|金氧半導體 金屬氧化物半導體|金鑰 密鑰|進程 進程|進階 高端|進位制 進制|晶片 芯片|巨集 宏|捲軸 滾動條|咖哩 咖喱|卡達 卡塔爾|鉲 鐦|開啟 打開|克羅埃西亞 克羅地亞|肯亞 肯尼亞|空間多工 空分複用|空氣清淨機 空氣淨化器|控制代碼 句柄|控制元件 控件|快捷半導體 仙童半導體|快取 緩存|快取記憶體 高速緩存|快閃記憶體 閃存|寬頻 寬帶|擴充套件 擴展|擴音 免提|來電轉接 呼叫轉移|賴比瑞亞 利比里亞|賴索托 萊索托|藍色畫面 藍屏|欄位 字段|雷射 激光|類比 模擬|類比電路 模擬電路|類比電子 模擬電子|類别範本 類模板|冷盤 涼菜|離線 脫機|李彥宏 李彥宏|例項 實例|連結 鏈接|連結串列 鏈表|連線 連接|聯結器 連接器|聯絡 聯繫|寮國 老撾|列舉 枚舉|列印 打印|列支敦斯登 列支敦士登|鎦 鑥|盧安達 盧旺達|錄影 錄像|邏輯閘 邏輯門|馬爾地夫 馬爾代夫|馬利共和國 馬里共和國|茅利塔尼亞 毛里塔尼亞|鋂 鎇|名稱空間 命名空間|名字空間 命名空間|命令列 命令行|模里西斯 毛里求斯|模擬 仿真|模組 模塊|摩爾線程 摩爾線程|莫三比克 莫桑比克|母音 元音|目的碼 目標代碼|那杜 溫納圖萬|奈及利亞 尼日利亞|奈米 納米|錼 鎿|內建 內置|內碼表 代碼頁|尼日 尼日爾|紐西蘭 新西蘭|諾魯 瑙魯|排程 調度|泡麵 方便麪|批次 批量|片語 詞組|頻寬 帶寬|平行計算 並行計算|破圖 花屏|蒲隆地 布隆迪|啟用 激活|氣泡排序 冒泡排序|千里達及托巴哥 特立尼達和多巴哥|簽帳金融卡 借記卡|前處理器 預處理器|喬治亞 格魯吉亞|區域網 局域網|區域性 局部|取樣 採樣|取樣率 採樣率|全球資訊網 萬維網|全形 全角|全域性 全局|人工智慧 人工智能|韌體 固件|乳酪 奶酪|入口網站 門戶網站|軟碟機 軟驅|軟體 軟件|塞普勒斯 塞浦路斯|塞席爾 塞舌爾|三極體 三極管|掃描器 掃描儀|沙烏地阿拉伯 沙特阿拉伯|傷心小棧 紅心大戰|尚比亞 贊比亞|燒錄 刻錄|社區 社區|社羣 社區|設定 設置|聖克里斯多福及尼維斯 聖基茨和尼維斯|聖露西亞 聖盧西亞|聖馬利諾 聖馬力諾|聖文森及格瑞那丁 聖文森特和格林納丁斯|獅子山 塞拉利昂|時脈頻率 時鐘頻率|實例 實例|實體地址 物理地址|實體記憶體 物理內存|識別符號 標識符|史瓦濟蘭 斯威士蘭|使用者 用戶|使用者名稱 用戶名|視窗 窗口|視覺化 可視化|視訊 視頻|視訊會議 視頻會議|視訊記憶體 顯存|視訊通話 視頻通話|釋出 發佈|數據機 調製解調器|數位 數字|數位電路 數字電路|數位電子 數字電子|數位印刷 數字印刷|數字 數字|斯洛維尼亞 斯洛文尼亞|搜尋 搜索|蘇利南 蘇里南|速食麵 方便麪|塑膠 塑料|隨身碟 U盤|縮排 縮進|縮圖 縮略圖|索羅門羣島 所羅門羣島|索馬利亞 索馬里|塔吉克 塔吉克斯坦|太空梭 航天飛機|坦尚尼亞 坦桑尼亞|跳脫字元 轉義字符|貼上 粘貼|停用 禁用|通道 信道|通過 通過|通話記錄 聯繫歷史|通話卡 通訊卡|通訊 通信|透過 通過|突尼西亞 突尼斯|圖示 圖標|土庫曼 土庫曼斯坦|吐瓦魯 圖瓦盧|外部索引鍵 外鍵|外掛 插件|外接 外置|萬那杜 瓦努阿圖|萬用字元 通配符|網際網路 互聯網|網咖 網吧|網路 網絡|網路上的芳鄰 網上鄰居|網絡卡 網卡|微控制器 單片機|維德角 佛得角|位元 比特|位元率 比特率|位元速率 碼率|位元組 字節|位址 地址|位址列 地址欄|文件 文檔|文書處理 文字處理|文字 文本|汶萊 文萊|烏茲別克 烏茲別克斯坦|無失真壓縮 無損壓縮|物件 對象|物件導向 面向對象|醯 酰|矽 硅|繫結 綁定|下拉式清單 下拉列表|夏農 香農|顯示卡 顯卡|線上 在線|相簿 圖庫|相容 兼容|向量 矢量|象牙海岸 科特迪瓦|消息 消息|效能 性能|擷取 截取|攜帶型 便攜式|辛巴威 津巴布韋|新增 添加|信號 信號|行動電話 移動電話|行動式 便攜式|行動通訊 移動通信|行動網路 移動網絡|行動硬碟 移動硬盤|行動資料 移動資料|行內函數 內聯函數|型別 類型|虛擬函式 虛函數|虛擬機器 虛擬機|虛擬碼 僞代碼|許可權 權限|序號產生器 註冊機|序列 串行|序列埠 串口|宣告 聲明|選單 菜單|循環 循環|訊號 信號|訊息 消息|訊雜比 信噪比|亞塞拜然 阿塞拜疆|演算法 算法|頁首 頁眉|頁尾 頁腳|葉門 也門|衣索比亞 埃塞俄比亞|乙太網 以太網|溢位 溢出|義大利 意大利|音效卡 聲卡|音訊 音頻|引數 參數|印表機 打印機|營運長 首席運營官|螢幕 屏幕|影片 視頻|影像 圖像|影印 複印|映象 鏡像|映象管 顯像管|硬碟 硬盤|硬體 硬件|永續性 持久性|永珍 萬象|優先順序 優先級|尤拉 歐拉|游標 光標|有失真壓縮 有損壓縮|預設 默認|預設值 默認值|元件 組件|原生代碼 本地代碼|原始檔 源文件|原始碼 原代碼|遠端 遠程|雲端儲存 雲存儲|雲端計算 雲計算|運算式 表達式|運算元 操作數|運算子 操作符|雜湊 哈希|載入 加載|載入程式 引導程序|暫存器 寄存器|閘道器 網關|閘電路 門電路|閘流體 晶閘管|粘貼 粘貼|遮蔽 屏蔽|摺積 捲積|真實模式 實模式|偵錯 調試|偵錯程式 調試器|陣列 數組|整合 集成|正規表示式 正則表達式|正規化 範式|支持者 支持者|支援 支持|直譯器 解釋器|執行 運行|執行緒 線程|執行長 首席執行官|指標 指針|指令碼 腳本|指令式程式設計 命令式編程|智慧 智能|智慧財產權 知識產權|中介軟體 中間件|終端使用者 最終用戶|重灌 重裝|重新命名 重命名|重新整理 刷新|主機板 主板|主開機記錄 主引導記錄|佇列 隊列|專案 項目|裝置 設備|狀態列 狀態欄|桌布 壁紙|桌上型 桌面型|桌上型電腦 臺式機|資料 數據|資料包 數據報|資料倉儲 數據倉庫|資料夾 文件夾|資料庫 數據庫|資料來源 數據源|資料探勘 數據挖掘|資訊 信息|資訊安全 信息安全|資訊科技 信息技術|資訊理論 信息論|資訊長 首席信息官|子音 輔音|字串 字符串|字節跳動 字節跳動|字首 前綴|字尾 後綴|字型 字體|字型檔 字庫|字元 字符|字元集 字符集|自動旋轉螢幕 自動轉屏|組合語言 彙編語言|組譯 彙編|最佳化 優化|作業系統 操作系統|PN接面 PN結|SQL隱碼攻擊 SQL注入";

var from_twp = [TWPhrasesRev, TWVariantsRevPhrases, TWVariantsRev];
...
var JPShinjitaiCharacters = "両 兩|弁 辨|欠 缺|浜 濱|糸 絲|芸 藝";

var JPVariantsRev = "奥 奧|抜 拔|覇 霸|拝 拜|浜 濱|宝 寶|痺 痹|辺 邊|弁 瓣|変 變|鼈 鱉|氷 冰|歩 步|才 纔|参 參|残 殘|蚕 蠶|惨 慘|厠 廁|曽 曾|挿 插|禅 禪|産 產|巣 巢|称 稱|乗 乘|痴 癡|遅 遲|歯 齒|勅 敕|虫 蟲|処 處|触 觸|床 牀|唇 脣|辞 辭|葱 蔥|聡 聰|従 從|醋 酢|粋 粹|帯 帶|担 擔|単 單|胆 膽|弾 彈|当 當|党 黨|祷 禱|盗 盜|稲 稻|徳 德|灯 燈|逓 遞|点 點|畳 疊|闘 鬥|独 獨|読 讀|断 斷|対 對|堕 墮|悪 惡|児 兒|弐 貳|発 發|髪 髮|払 拂|仮 假|廃 廢|峰 峯|仏 佛|缶 罐|皐 皋|亘 亙|掴 摑|関 關|観 觀|広 廣|帰 歸|亀 龜|国 國|号 號|黒 黑|恒 恆|横 橫|戯 戲|戸 戶|沪 濾|画 畫|懐 懷|壊 壞|歓 歡|黄 黃|会 會|恵 惠|絵 繪|撃 擊|鶏 雞|剤 劑|済 濟|継 繼|曁 暨|繋 繫|倹 儉|検 檢|鹸 鹼|剣 劍|践 踐|将 將|蒋 蔣|奨 奬|醤 醬|脚 腳|撹 攪|掲 揭|桝 枡|届 屆|尽 盡|晋 晉|茎 莖|経 經|径 徑|浄 淨|静 靜|旧 舊|挙 舉|拠 據|巻 卷|絶 絕|覚 覺|渇 渴|寛 寬|鉱 鑛|拡 擴|蝋 蠟|来 來|莱 萊|頼 賴|瀬 瀨|覧 覽|労 勞|楽 樂|塁 壘|涙 淚|稜 棱|礼 禮|豊 豐|励 勵|茘 荔|砺 礪|暦 曆|歴 歷|連 聯|恋 戀|錬 鍊|両 兩|輌 輛|猟 獵|霊 靈|齢 齡|竜 龍|滝 瀧|楼 樓|芦 蘆|炉 爐|禄 祿|録 錄|緑 綠|乱 亂|売 賣|麦 麥|蛮 蠻|満 滿|猫 貓|没 沒|毎 每|弥 彌|糸 絲|秘 祕|麺 麪|黙 默|悩 惱|脳 腦|内 內|嬢 孃|醸 釀|噛 嚙|欧 歐|殴 毆|鴎 鷗|溌 潑|醗 醱|舗 鋪|斉 齊|気 氣|銭 錢|潜 潛|浅 淺|欠 缺|殻 殼|窃 竊|寝 寢|区 區|駆 驅|麹 麴|圏 圈|権 權|勧 勸|却 卻|群 羣|穣 穰|壌 壤|譲 讓|栄 榮|掻 搔|騒 騷|渋 澀|刹 剎|焼 燒|舎 舍|渉 涉|摂 攝|声 聲|縄 繩|剰 剩|湿 溼|実 實|釈 釋|収 收|寿 壽|痩 瘦|獣 獸|枢 樞|疏 疎|属 屬|数 數|双 雙|税 稅|説 說|価 價|捜 搜|粛 肅|随 隨|髄 髓|砕 碎|歳 歲|穂 穗|台 臺|涛 濤|体 體|戻 戾|条 條|庁 廳|聴 聽|囲 圍|図 圖|団 團|脱 脫|弯 彎|湾 灣|晩 晚|万 萬|偽 僞|為 爲|温 溫|穏 穩|汚 污|呉 吳|渓 溪|犠 犧|峡 峽|狭 狹|繊 纖|険 險|顕 顯|県 縣|陥 陷|献 獻|郷 鄉|暁 曉|効 效|挟 挾|写 寫|繍 繡|虚 虛|嘘 噓|叙 敘|続 續|緒 緖|学 學|勲 勳|薫 薰|圧 壓|唖 啞|亜 亞|煙 菸|塩 鹽|厳 嚴|顔 顏|巌 巖|彦 彥|焔 焰|験 驗|贋 贗|様 樣|揺 搖|遙 遥|謡 謠|薬 藥|医 醫|壱 壹|訳 譯|駅 驛|隠 隱|応 應|桜 櫻|営 營|蛍 螢|頴 穎|余 餘|楡 榆|与 與|予 豫|御 禦|誉 譽|円 圓|縁 緣|岳 嶽|悦 悅|閲 閱|匀 勻|伝 傳|芸 藝|雑 雜|賛 贊|蔵 藏|臓 臟|竃 竈|択 擇|沢 澤|増 增|査 查|斎 齋|桟 棧|戦 戰|争 爭|徴 徵|証 證|鉄 鐵|滞 滯|軽 輕|呪 咒|昼 晝|嘱 囑|鋳 鑄|専 專|転 轉|庄 莊|荘 莊|粧 妝|装 裝|壮 壯|状 狀|総 總|粽 糉|縦 縱|酔 醉";

var from_jp = [JPShinjitaiPhrases, JPShinjitaiCharacters, JPVariantsRev];

var TSPhrases = "一目瞭然 一目了然|上鍊 上链|不瞭解 不了解|么麼 幺麽|么麽 幺麽|乾乾淨淨 干干净净|乾乾脆脆 干干脆脆|乾元 乾元|乾卦 乾卦|乾嘉 乾嘉|乾圖 乾图|乾坤 乾坤|乾坤一擲 乾坤一掷|乾坤再造 乾坤再造|乾坤大挪移 乾坤大挪移|乾宅 乾宅|乾斷 乾断|乾旦 乾旦|乾曜 乾曜|乾清宮 乾清宫|乾盛世 乾盛世|乾紅 乾红|乾綱 乾纲|乾縣 乾县|乾象 乾象|乾造 乾造|乾道 乾道|乾陵 乾陵|乾隆 乾隆|乾隆年間 乾隆年间|乾隆皇帝 乾隆皇帝|二噁英 二𫫇英|以免藉口 以免借口|以功覆過 以功复过|侔德覆載 侔德复载|傢俱 家具|傷亡枕藉 伤亡枕藉|八濛山 八濛山|凌藉 凌借|出醜狼藉 出丑狼藉|函覆 函复|千鍾粟 千锺粟|反反覆覆 反反复复|反覆 反复|反覆思維 反复思维|反覆思量 反复思量|反覆性 反复性|名覆金甌 名复金瓯|哪吒 哪吒|回覆 回复|壺裏乾坤 壶里乾坤|大目乾連冥間救母變文 大目乾连冥间救母变文|宫商角徵羽 宫商角徵羽|射覆 射复|尼乾子 尼乾子|尼乾陀 尼乾陀|幺麼 幺麽|幺麼小丑 幺麽小丑|幺麼小醜 幺麽小丑|康乾 康乾|張法乾 张法乾|彷彿 仿佛|彷徨 彷徨|徵弦 徵弦|徵絃 徵弦|徵羽摩柯 徵羽摩柯|徵聲 徵声|徵調 徵调|徵音 徵音|情有獨鍾 情有独钟|憑藉 凭借|憑藉着 凭借着|手鍊 手链|扭轉乾坤 扭转乾坤|找藉口 找借口|拉鍊 拉链|拉鍊工程 拉链工程|拜覆 拜复|據瞭解 据了解|文錦覆阱 文锦复阱|於世成 於世成|於乎 於乎|於仲完 於仲完|於倫 於伦|於其一 於其一|於則 於则|於勇明 於勇明|於呼哀哉 於呼哀哉|於單 於单|於坦 於坦|於崇文 於崇文|於忠祥 於忠祥|於惟一 於惟一|於戲 於戏|於敖 於敖|於梨華 於梨华|於清言 於清言|於潛 於潜|於琳 於琳|於穆 於穆|於竹屋 於竹屋|於菟 於菟|於邑 於邑|於陵子 於陵子|旋乾轉坤 旋乾转坤|旋轉乾坤 旋转乾坤|旋轉乾坤之力 旋转乾坤之力|明瞭 明了|明覆 明复|書中自有千鍾粟 书中自有千锺粟|有序 有序|朝乾夕惕 朝乾夕惕|木吒 木吒|李乾德 李乾德|李澤鉅 李泽钜|李鍊福 李链福|李鍾郁 李锺郁|樊於期 樊於期|沈沒 沉没|沈沒成本 沉没成本|沈積 沉积|沈船 沉船|沈默 沉默|流徵 流徵|浪蕩乾坤 浪荡乾坤|滑藉 滑借|無序 无序|牴牾 抵牾|牴觸 抵触|狐藉虎威 狐借虎威|珍珠項鍊 珍珠项链|甚鉅 甚钜|申覆 申复|畢昇 毕昇|發覆 发复|瞭如 了如|瞭如指掌 了如指掌|瞭望 瞭望|瞭然 了然|瞭然於心 了然于心|瞭若指掌 了若指掌|瞭解 了解|瞭解到 了解到|示覆 示复|神祇 神祇|稟覆 禀复|竺乾 竺乾|答覆 答复|篤麼 笃麽|簡單明瞭 简单明了|籌畫 筹划|素藉 素借|老態龍鍾 老态龙钟|肘手鍊足 肘手链足|茵藉 茵借|萬鍾 万锺|蒜薹 蒜薹|蕓薹 芸薹|蕩覆 荡复|蕭乾 萧乾|藉代 借代|藉以 借以|藉助 借助|藉助於 借助于|藉卉 借卉|藉口 借口|藉喻 借喻|藉寇兵 借寇兵|藉寇兵齎盜糧 借寇兵赍盗粮|藉手 借手|藉據 借据|藉故 借故|藉故推辭 借故推辞|藉方 借方|藉條 借条|藉槁 借槁|藉機 借机|藉此 借此|藉此機會 借此机会|藉甚 借甚|藉由 借由|藉着 借着|藉端 借端|藉端生事 借端生事|藉箸代籌 借箸代筹|藉草枕塊 借草枕块|藉藉 藉藉|藉藉无名 藉藉无名|藉詞 借词|藉讀 借读|藉資 借资|衹得 只得|衹見樹木 只见树木|衹見樹木不見森林 只见树木不见森林|袖裏乾坤 袖里乾坤|覆上 复上|覆住 复住|覆信 复信|覆冒 复冒|覆呈 复呈|覆命 复命|覆墓 复墓|覆宗 复宗|覆帳 复帐|覆幬 复帱|覆成 复成|覆按 复按|覆文 复文|覆杯 复杯|覆校 复校|覆瓿 复瓿|覆盂 复盂|覆盆 覆盆|覆盆子 覆盆子|覆盤 覆盘|覆育 复育|覆蕉尋鹿 复蕉寻鹿|覆逆 复逆|覆醢 复醢|覆醬瓿 复酱瓿|覆電 复电|覆露 复露|覆鹿尋蕉 复鹿寻蕉|覆鹿遺蕉 复鹿遗蕉|覆鼎 复鼎|見覆 见复|角徵 角徵|角徵羽 角徵羽|計畫 计划|變徵 变徵|變徵之聲 变徵之声|變徵之音 变徵之音|貂覆額 貂复额|買臣覆水 买臣复水|踅門瞭戶 踅门了户|躪藉 躏借|郭子乾 郭子乾|酒逢知己千鍾少 酒逢知己千锺少|酒逢知己千鍾少話不投機半句多 酒逢知己千锺少话不投机半句多|醞藉 酝借|重覆 重复|金吒 金吒|金鍊 金链|鈞覆 钧复|鉅子 钜子|鉅萬 钜万|鉅防 钜防|鉸鍊 铰链|銀鍊 银链|錢鍾書 钱锺书|鍊墜 链坠|鍊子 链子|鍊形 链形|鍊條 链条|鍊錘 链锤|鍊鎖 链锁|鍛鍾 锻锺|鍾繇 钟繇|鍾萬梅 锺万梅|鍾重發 锺重发|鍾鍛 锺锻|鍾馗 锺馗|鎖鍊 锁链|鐵鍊 铁链|鑽石項鍊 钻石项链|雁杳魚沈 雁杳鱼沉|雖覆能復 虽覆能复|電覆 电复|露覆 露复|項鍊 项链|頗覆 颇复|頸鍊 颈链|顛乾倒坤 颠乾倒坤|顛倒乾坤 颠倒乾坤|顧藉 顾借|麼些族 麽些族|黄鍾公 黄锺公|龍鍾 龙钟";
...
var to_cn = [TSPhrases, TSCharacters];
...
export { Converter, ConverterFactory, CustomConverter, HTMLConverter, Locale, Trie };
